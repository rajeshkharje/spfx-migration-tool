...
namespace BART.SP.OCR.CP.Model
{
    ...
    public class Attachment
    {
        ...
        public Attachment(string masterid = "")
        {
            if (!string.IsNullOrEmpty(masterid))
                this.MasterID = masterid;
        }

        public Attachment(SPListItem item)
        {
            InitObject(item);
        }
        public Attachment(SPWeb dWeb, string itemId)
        {
            ...
            try
            {
                if (!string.IsNullOrWhiteSpace(photoFile.FileName))
                {
                    ...
                    if (fStream.Length <= Settings.FileSizeLimit)
                    {
                        ...
                        SPFile fileAdded = dWeb.Lists[Common.ProjectSettings.SPListAttachment].RootFolder.Files.Add(filename, contents, true);
